...
class Board:
    ...
    def draw(self, window):  # draw the board on window
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.bo[i][j] == "O":
                    pygame.draw.circle(
                        window, BLACK, (j * len + len//2, i * len + len//2), len // 3, 3)
                if self.bo[i][j] == "X":
                    pygame.draw.line(window, BLACK, (len // 5 + j * len, i * len + len // 5),
                                     (j * len + len - len // 5, i * len + len - len // 5), 3)
                    pygame.draw.line(window, BLACK, (j * len + len - len // 5, i *
                                                     len + len // 5), (j * len + len // 5, i * len + len - len // 5), 3)
    ...
    def assign(self, shape, i, j):
        if (not self.is_available(i, j)):
            raise PositionTaken(
                "This position is taken. Choose a different one")
        else:
            self.bo[i][j] = shape
            self.empty_places -= 1
    ...
    # Player [shape] chose bo[i][j] pos. check if he wins
    def check_for_winner(self, player, i, j):
        len = WIDTH // self.size
        ...
        if self.bo[i] == winner_seq:  # check row
            ...
            return True
        # check col
        elif [self.bo[h][j] for h in range(self.size)] == winner_seq:
            ...
            if i == j:
                # check main diagonal
                if [self.bo[h][h] for h in range(self.size)] == winner_seq:
                ...
                if [self.bo[self.size - 1 - h][h] for h in range(self.size)] == winner_seq:
