...
def redraw(win, game, p):
    ...
    if not game.connected():
        ...
        win.blit(waiting_label, (win.get_width() / 2 - waiting_label.get_width() / 2, win.get_height() / 2 - waiting_label.get_height() / 2))
    else:
        game.board.draw(win)
        winner = game.get_winner()
        if winner != None: #The game ended
            pygame.draw.line(win, BLACK, game.get_winner_line_start(), game.get_winner_line_end(), 4)
            if winner == p: #We won
                winner_label = FONT.render("You Won :)", 1, (255, 0, 0))
                win.blit(winner_label, (win.get_width() // 2 - winner_label.get_width() // 2, win.get_height() // 2 - 15))
            else: #we lost
                loser_label = FONT.render("You Lost :(", 1, (255, 0, 0))
                win.blit(loser_label, (win.get_width() // 2 - loser_label.get_width() // 2, win.get_height() // 2 - 15))
        elif game.is_draw():
            draw_label = FONT.render("Draw!", 1, (255, 0, 0))
            win.blit(draw_label, (win.get_width() // 2 - draw_label.get_width() // 2, win.get_height() // 2 - 15))
...
def menu_screen():
    ...
    while run:
        ...
        window.blit(start_label, (window.get_width() // 2 - start_label.get_width() // 2, window.get_height() // 2 - 15) )
        pygame.display.update()
